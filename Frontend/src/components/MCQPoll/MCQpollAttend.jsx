...
import { Box, Heading, Text, VStack, Button, Radio, RadioGroup, Checkbox, CheckboxGroup, useToast } from '@chakra-ui/react';
  ...
  const handleCheckboxChange = (questionId, optionId, isChecked) => {
    setSelectedOptions((prevState) => {
      ...
      if (isChecked) {
        ...
        return {
          ...prevState,
          [questionId]: selectedOptionsForQuestion.filter((id) => id !== optionId),
        };
      }
    });
  };
    ...
    try {
      const payload = {
        answers: poll.questions.map((question) => {
          if (question.type === 'checkbox') {
            const selectedOptionsForQuestion = selectedOptions[question._id] || [];
            return {
              questionId: question._id,
              questionText: question.question,
              answerId: selectedOptionsForQuestion,
              answerText: selectedOptionsForQuestion.map((optionId) => question.options.find((option) => option._id === optionId).text),
            };
          } else {
            return {
              questionId: question._id,
              ...
              >
                {question.options.map((option, idx) => (
                  <Checkbox
                    key={idx}
                    value={option._id}
                    isChecked={selectedOptions[question._id]?.includes(option._id)}
                    onChange={(e) => handleCheckboxChange(question._id, option._id, e.target.checked)}
                  >
                    {option.text}
                  </Checkbox>
                ))}

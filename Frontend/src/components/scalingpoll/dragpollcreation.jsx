...
const CreatePollDrag = () => {
  ...
  const [questions, setQuestions] = useState([{ question: '', useSlider: true, options: Array.from({ length: 10 }, (_, i) => ({ text: `${i + 1}` })) }]);
  ...
  const handleUseSliderChange = (index) => {
    ...
    if (newQuestions[index].useSlider) {
      newQuestions[index].options = Array.from({ length: 10 }, (_, i) => ({ text: `${i + 1}` }));
    } else {
  ...
  const handleAddQuestion = () => {
    setQuestions([...questions, { question: '', useSlider: true, options: Array.from({ length: 10 }, (_, i) => ({ text: `${i + 1}` })) }]);
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      alert("Poll Created");
      setTitle("");
      setQuestions([{ question: '', useSlider: true, options: Array.from({ length: 10 }, (_, i) => ({ text: `${0 + 1}` })) }]);
      ...
      console.error('Error creating poll:', error.response?.data || error.message || error);
    }
  };
              ...
              <>
                <InputGroup mt={2}>
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const newQuestions = [...questions];
                      newQuestions[index].options = e.target.value.split(',').map(text => ({ text: text.trim() }));
                      setQuestions(newQuestions);
                    }}
                  />
                </InputGroup>

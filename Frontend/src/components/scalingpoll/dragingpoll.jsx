...
const RatingPollAttend = () => {
  ...
  const handleSubmit = async () => {
    ...
    if (responses.length !== poll.questions.length || textResponses.length !== poll.questions.length) {
    ...
    try {
      const payload = {
        ...
        answers: poll.questions.map((question, index) => ({
          ...
          selectedOption: question.useSlider ? responses[index] : textResponses[index]
        }))
      };
  ...
  return (
    <Box w="100%" maxW="600px" mx="auto" p={4} borderWidth={1} borderRadius="lg" boxShadow="lg">
      <Heading as="h2" size="xl" mb={4}>
        {poll.title}
      </Heading>
      <VStack spacing={4}>
        {poll.questions.map((question, index) => (
          <Box key={question._id} w="100%" p={4} borderWidth={1} borderRadius="lg">
